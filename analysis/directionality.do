...
* (4) The four side x regime cells within directional-held bonds only (hf_dir3 == 2).
* Compare the short vs long level with the same four cells on all HF-held bonds. If the
* gap shrinks here, it is the directionality result. Rerun with hf_dir3 == 1 for hedged-held.

* long, loss
reghdfe delta_y c.log_hf_intensity##c.ois_2y bid_ask_spread ctd_flag ///
    if (ois_2y > 0 & hf_intensity_long > 0 & hf_dir3 == 2) ///
 | (ois_2y == 0) ///
 | (hf_intensity_pre == 0), ///
    absorb(duration_match isin) vce(cluster business_date isin)
...
* short, loss
reghdfe delta_y c.log_hf_intensity##c.ois_2y bid_ask_spread ctd_flag ///
    if (ois_2y < 0 & hf_intensity_short > 0 & hf_dir3 == 2) ///
 | (ois_2y == 0) ///
 | (hf_intensity_pre == 0), ///
    absorb(duration_match isin) vce(cluster business_date isin)
