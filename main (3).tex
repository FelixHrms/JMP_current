...
I use granular regulatory data on euro area repo transactions to show that the leveraged positioning of hedge funds amplifies the response of sovereign bond yields to shocks. An average monetary policy surprise of six basis points moves the yields of bonds traded by hedge funds by almost two basis points more than the yields of comparable bonds, so more than a quarter of the total reaction on shock days reflects hedge fund positioning rather than the fundamental news itself. This overshooting is temporary and reverts over the following trading days. Its strength depends not only on the size of the position but also on the directionality of the hedge fund book that holds it.

The mechanism is general and applies whenever a price-moving event interacts with pre-existing leveraged positions. Measuring it, however, requires both detailed position data and a clean source of exogenous price variation. To address the first requirement, I use a dataset that provides a transaction-level view of the euro area repo market and allows me to precisely track the daily positioning of hedge funds across individual sovereign bonds. To address the second, I combine this micro data with bond market data and high-frequency monetary policy surprises, which provide a laboratory for studying how leveraged positions react to exogenous price changes. By interacting the pre-determined net positions of hedge funds with the shock, I estimate the additional yield sensitivity that arises when hedge funds are trading a given bond. I then use the full cross section of each fund's positions to show that the amplification depends on whether the bond sits within a hedged or directional book.

Hedge funds have become increasingly important players in global financial markets, and how this growing footprint affects the sensitivity of asset prices to shocks can be viewed from two perspectives. The first emphasizes their arbitrage activity. By correcting mispricings and providing liquidity, hedge funds push prices toward fundamental value and dampen the reaction to shocks \citep{gromb2002, cao2018}. The second emphasizes the risks arising from their reliance on leverage. When funding conditions deteriorate, forced liquidations can push prices further from fundamentals rather than closer to them, amplifying the reaction \citep{shleifer1997, brunnermeier2009, geanakoplos2010}. Attention to the second perspective has grown since the global financial crisis, as tighter regulation of traditional banks has shifted intermediation toward non-bank financial intermediaries \citep{pelizzon2025nbfi}. Among these, offshore hedge funds stand out for their opacity and strong use of leverage \citep{barth2025cross}. They finance their positions with short-term repurchase agreements, contracts in which a security is sold with a commitment to repurchase it at a later date, which makes them highly sensitive to sudden changes in the value of collateral. This fragility surfaced in March 2020, when forced deleveraging by hedge funds exacerbated liquidity stress in the US Treasury market \citep{barth2025, kruttli2025, aramonte2023}. Yet granular data on their repo positioning has been largely unavailable, and the existing evidence is concentrated in stress episodes and focused on market functioning rather than the cross-sectional pricing of bonds. This gap leaves open whether leveraged non-bank trading shapes price formation outside of crises, and what kind of positioning drives the effect when it does.

I document four main results on the role of hedge fund positioning in sovereign bond markets. First, hedge funds do select more volatile bonds, but the additional volatility due to their activity appears only on shock days. Under duration-by-country-by-date fixed effects, hedge fund bonds are no more volatile than comparable bonds on regular days, while hedge fund involvement still raises volatility on the days when monetary policy surprises arrive. The unconditional differential therefore reflects the characteristics of the bonds hedge funds select, whereas the shock-day increment reflects their positioning.

Second, hedge fund trading significantly amplifies the sensitivity of sovereign bond yields to monetary policy surprises, which is what the shock-day volatility in the first result picks up. This amplification generates the additional yield movement reported above, and it scales with the size of the position, as yield sensitivity increases with the fraction of a bond held by hedge funds within the same bond over time. This dose response is consistent with a mechanical link between the volume of leveraged trading and the resulting price dislocation, and it provides an identification argument of its own, since within-bond variation cannot be explained by the selection of special bonds. Local projections show that the divergence appears immediately on the impact day, persists for approximately four trading days, and then reverts back toward zero. The temporary nature of the overshooting indicates that hedge fund activity moves prices away from fundamentals in the short run, more consistent with the amplification view of leveraged non-bank trading than with the price-discovery view.
...
A first challenge in identifying the impact of hedge fund positioning is selection bias, and the first result shows that this concern is warranted. Hedge funds do not select bonds randomly, but strategically target bonds with characteristics such as high liquidity and special status as the cheapest-to-deliver against an active futures contract, characteristics that may independently affect how strongly the bond reacts to a shock. I address this concern by introducing progressively tighter fixed effect specifications, up to a specification with duration-by-country-by-date fixed effects that compares bonds of similar duration from the same issuer on the same day, and by implementing a matched-sample design that pairs each hedge fund bond with the single non-hedge fund bond in the same duration-country-date cell that is closest in terms of liquidity. Together, these specifications help isolate the effect of leveraged positioning from the characteristics that make bonds attractive to hedge funds in the first place.

A second challenge is that any amplification could reflect not hedge fund positioning itself but rather the behavior of the dealers that intermediate their trades. If dealers respond to shocks in ways that systematically differ across the bonds where they maintain active hedge fund relationships, the results could capture dealer behavior rather than the leveraged positioning of the hedge funds themselves. To provide evidence against this explanation, I decompose the bond-date panel into dealer-bond-date observations and introduce increasingly demanding dealer fixed effect structures. Additionally, I use credit default swaps to estimate whether the effect scales with dealer stress. The analysis confirms that the effect operates through hedge fund positioning, rather than the behavior of dealer banks.
...
This paper uses a transaction-level dataset to document evidence for a non-bank amplification channel in sovereign bond markets. Hedge funds do select more volatile bonds, but once bonds of similar duration from the same issuer are compared, the additional volatility of their holdings appears only on shock days. The leveraged positioning of offshore hedge funds amplifies the sensitivity of sovereign bond yields to monetary policy shocks, accounting for more than a quarter of the total yield movement on shock days, and the amplification scales with the size of the position relative to the bond's outstanding volume. It operates with comparable strength in both the constraining regime and the relaxing regime, and local projections of hedge fund positioning provide evidence that the two regimes correspond to position contractions and expansions as the underlying mechanism predicts. The amplification is concentrated among bonds held within directional hedge fund books and is essentially absent when the book offsets its long and short positions, which places the binding constraint at the level of the fund's portfolio rather than the individual position.
