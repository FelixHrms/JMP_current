...
This section describes the framework that guides the empirical analysis. It traces how a price shock to a leveraged position affects the capital of the fund and, through the resulting position adjustment, the yield of the bond. The framework is intended to motivate the empirical design rather than to serve as a formal model. The tests below are designed along its predictions, and they are not meant to establish the framework as the only account of the results. Appendix \ref{subsec:calibration} presents a calibrated version of the framework based on sample moments.

\citet{adrian2014} provide the broader intuition that leveraged intermediaries actively manage their balance sheets in response to the risk environment, expanding positions when constraints are loose and contracting them when constraints bind. The key insight is that the state of the constraint environment can generate symmetric amplification effects with opposite signs of position adjustment depending on the direction of the shock. While \citet{adrian2014} focus on US broker-dealers, I apply this logic to hedge funds, where the constraint environment shifts directly with the value of the collateral backing their positions.
...
For a given margin requirement, the amount of capital caps the position a fund can hold, so a loss or gain of capital lowers or raises that cap by the same proportion. A shock that moves prices against the position erodes the buffer and tightens the constraint, forcing position reduction. A shock that moves prices in favor of the position relaxes the constraint and frees capacity for additional risk-taking. Both responses are thus governed by the same rule, with the sign of the shock in combination with the position of the fund determining whether positions contract or expand. In both cases, the resulting trading pushes yields further in the direction of the initial shock. The two sides differ in their nature, however. The contraction is enforced by the requirement to keep meeting margin, while the expansion is voluntary. 

The contraction is the deleveraging response studied in the literature on funding constraints, in which losses erode capital and force position reductions \citep{brunnermeier2009, geanakoplos2010}. The expansion is its counterpart under the balance sheet management of \citet{adrian2014}, in which freed capacity is redeployed into the existing exposure. Such redeployment is also the optimal response of a constrained arbitrageur whose desired position exceeds its funding capacity, because a favorable surprise enlarges the capacity while the remaining expected return still justifies a larger position than the enlarged balance sheet permits \citep{gromb2002}, and hedge funds are indeed documented to raise leverage when asset values rise and funding costs fall \citep{ang2011}. Appendix \ref{subsec:calibration} works through a numerical example of both sides.
