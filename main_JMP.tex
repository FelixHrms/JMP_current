...
The framework builds on \citet{adrian2014}, who describe leveraged intermediaries that manage their balance sheets toward a target leverage ratio. When asset values rise, leverage falls below the target and the intermediary expands its positions. When asset values fall, leverage rises above the target and the intermediary contracts them. The adjustment is therefore two-sided, and the direction of the shock determines the sign of the position adjustment. While \citet{adrian2014} focus on US broker-dealers, whose target follows from a value at risk constraint, I apply this logic to hedge funds that finance bond positions through repo. For these funds, the margin requirement on the collateral sets the maximum leverage of the position, and a fund that operates at this maximum has its target set by the requirement.
...
\label{eq:capital}
\end{equation}
Shocks of the size observed in the sample have a substantial effect on the capital buffer. The average tightening surprise in the sample is approximately 6 basis points, which moves the price of a representative bond with a duration of about seven years by roughly 0.42\%. Under an effective margin of 2\%, which lies well above the haircuts reported in Table \ref{tab:summary_stats} and is therefore a conservative assumption, Equation (\ref{eq:capital}) implies that an average surprise moves the capital buffer by about 21\%. Because the value of the position moves far less than the capital backing it, the leverage of the position moves in the opposite direction, above the target after a loss and below it after a gain.

Returning to the target requires a position adjustment of the same proportion as the change in capital. After a loss, the fund must reduce the position to restore the margin ratio, and the reduction is enforced by the margin call. After a gain, the fund can expand the position to the new capacity, and the expansion is voluntary. The contraction is the deleveraging response studied in the literature on funding constraints, in which losses erode capital and force position reductions \citep{brunnermeier2009, geanakoplos2010}. The expansion follows if the fund manages its leverage to the target in the sense of \citet{adrian2014}. It is also the optimal response of a constrained arbitrageur whose desired position exceeds its funding capacity, because a favorable surprise enlarges the capacity while the remaining expected return still justifies a larger position than the enlarged balance sheet permits \citep{gromb2002}, and hedge funds are documented to raise leverage when asset values rise and funding costs fall \citep{ang2011}. Appendix \ref{subsec:calibration} works through a numerical example of both sides. In both cases, the position adjustment is in the direction of the initial shock, which for a long position means a sale after a price fall and a purchase after a price rise.

The yield reaction to the position adjustment depends on the market's ability to absorb it. The trading pressure equals the proportional adjustment multiplied by the share of the bond that the hedge fund sector holds, and its price impact depends on the depth of the market for that bond. The adjustment takes place within a short period after the shock, since margin calls are settled daily and the framework assumes that freed capacity is redeployed promptly rather than over months. The trading pressure is therefore concentrated in the days following the shock, and it reflects the return to the target rather than a change in fundamentals.

In sum, the framework implies that bonds in which hedge funds hold leveraged positions react more strongly to a shock than comparable bonds, the more so the larger the position relative to the amount outstanding, that the additional reaction is temporary and reverts as other capital absorbs the imbalance, and that it arises for shocks in both directions, with a position contraction when the shock moves prices against the position and an expansion when it moves in its favor. Section \ref{subsec:amplification} examines the first two implications and Section \ref{subsec:asymmetries} the third. The results are read as consistent with the framework rather than as establishing it, and Section \ref{subsec:asymmetries} returns to the question of alternative explanations.
