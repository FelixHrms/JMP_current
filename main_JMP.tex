...
This section lays out the interpretation that organizes the empirical analysis. It is not a model. Its purpose is to motivate the sample splits used below and to fix the language in which the results are discussed, and the contribution of the paper lies in the measurement rather than in the framework. One distinction matters throughout. The data test the same thing on both sides, whether positions adjust in the predicted direction and whether yields amplify, and they identify the motive on neither. What differs is the prior. When a shock moves prices against the position, the margin call predicts the contraction without any assumption about the fund's objective. When a shock moves prices in its favor, the expansion requires the assumption that the fund's desired position exceeds what its balance sheet permits. Appendix \ref{subsec:calibration} works through a calibrated example of both sides with sample moments.
...
The magnitudes involved are not small. The average tightening surprise in the sample is approximately 6 basis points, which moves the price of a representative bond with a duration of about seven years by roughly 0.42\%. Under an effective margin of 2\%, which is conservative relative to the haircuts reported in Table \ref{tab:summary_stats}, Equation (\ref{eq:capital}) implies that a single surprise moves the capital buffer by about a fifth.
...
Appendix \ref{subsec:calibration} benchmarks this magnitude against a calibrated version of the framework in Section \ref{sec:mechanism}. With the haircuts and holdings of the sample and market-depth estimates from the literature, the position adjustment implied by a six basis point surprise produces an additional yield move of roughly 3 basis points, which is the same order of magnitude as the 1.73 basis points estimated here.
...
The size of the expansion deserves a remark. Relative to the gain in capital, the fund adds fifty times as much bond as it earned, which can look excessive at first sight. The multiplier is the leverage ratio $1/m_i$, and it is the same multiplier that turns a 0.42\% price loss into a 21\% position reduction on the constraining side. The symmetry of the response therefore follows from the leverage of the position and not from an additional assumption.

Finally, the calibrated amplification can be compared with the estimates of Section \ref{subsec:amplification}. The binary interaction coefficient of 0.288 in Table \ref{tab:baseline} implies an additional yield move of approximately 1.73 basis points for a 6 basis point surprise, and the intensity specification in Table \ref{tab:intensity} implies approximately 1.36 basis points at an average intensity of 3\%. The calibrated value of 3.15 basis points is of the same order of magnitude and lies within a factor of two of the estimates. Given the stylized parameters, in particular the effective margin and the market elasticity, the example confirms that the mechanism can produce amplification of the observed size, and it should not be read as a quantitative prediction. A higher effective margin or a deeper market would lower the calibrated value toward the estimates.
