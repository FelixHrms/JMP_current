...
Table \ref{tab:fimacro} re-estimates the main specifications with hedge fund positioning measured exclusively from funds classified as fixed income relative value or global macro in Appendix \ref{subsec:classification}. This addresses the concern that repo positions of other fund types need not reflect sovereign bond trades. A multi-strategy fund, for example, may post a bond it already holds in repo and use the cash to finance positions in other asset classes, in which case its repo activity proxies for funding demand rather than directional bond exposure. All positioning variables are recomputed from the transactions of fixed income and macro funds only, and bond-days on which only hedge funds of other strategy types are active are excluded from the sample rather than treated as controls. The estimated coefficients are nearly identical to the main specifications across the binary, continuous intensity, constraining, and relaxing regime tests. The amplification is therefore driven by funds that use the repo market to run leveraged sovereign bond positions. \\
...
Duration, the characteristic that governs a bond's mechanical sensitivity to rate shocks, is closely balanced, with a standardized difference of 0.03. The remaining rows reflect the selection documented in Table \ref{tab:hf_selection}. Hedge fund bonds are more liquid than their matched controls, are concentrated in larger issues, and carry a somewhat longer residual maturity at the same duration, which reflects coupon differences rather than differences in rate exposure. Because the hedge fund bond is typically the most liquid bond of its cell, the matched control is the closest feasible comparison but remains less liquid in the average pair. With more than 240,000 pairs, even small differences are statistically significant, so the standardized differences are the informative metric. The two bonds differ in cheapest-to-deliver status in 6.4\% of pairs. CTD status is controlled for in all specifications, and Appendix \ref{subsec:ctd} shows that the results hold when CTD bonds are excluded entirely. \\
...
\begin{tabular}{l c c c c c c}
\toprule
 & HF Bond & Non-HF Bond & Mean Diff. & Median $|$Diff.$|$ & Std. Diff. & p-value \\
\midrule
Duration (years) & 5.46 & 5.31 & 0.15 & 0.55 & 0.03 & $<$0.01 \\
Residual Maturity (years) & 6.66 & 5.39 & 1.27 & 0.79 & 0.20 & $<$0.01 \\
Amount Issued (\euro\ bn) & 19.81 & 11.36 & 8.45 & 8.28 & 1.43 & $<$0.01 \\
Bid-Ask Spread (\euro) & 0.08 & 0.20 & $-$0.13 & 0.05 & $-$0.83 & $<$0.01 \\
\midrule
Pairs & \multicolumn{6}{c}{240,912} \\
Distinct HF bonds & \multicolumn{6}{c}{471} \\
Distinct non-HF bonds & \multicolumn{6}{c}{582} \\
Distinct dates & \multicolumn{6}{c}{1,242} \\
CTD mismatch rate & \multicolumn{6}{c}{6.4\%} \\
\bottomrule
\end{tabular}
\label{tab:pair_balance}
\caption*{\textit{Notes:} This table reports balance statistics for the matched sample used in Column (5) of Table \ref{tab:baseline}. Each hedge fund bond-day is paired with the non-hedge fund bond in the same duration-country-date cell that is closest in terms of bid-ask spread, with replacement. \textit{HF Bond} and \textit{Non-HF Bond} report the means of each characteristic across the two sides of the pairs. \textit{Mean Diff.} and \textit{Median $|$Diff.$|$} refer to the within-pair difference (HF minus non-HF) and the absolute within-pair difference. \textit{Std. Diff.} is the mean difference divided by the square root of the average of the two group variances. The p-value tests the mean within-pair difference against zero, with standard errors clustered by business date. The \textit{CTD mismatch rate} is the share of pairs in which the two bonds differ in cheapest-to-deliver status. \\ Sample period: 2021-01-01 -- 2025-11-01}
\end{table}
...
\begin{table}[h!]
\centering
\caption{\textbf{Pair Quality and Bond Characteristics in the Matched Sample}}
\footnotesize
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{l c c c c c c}
...
\begin{table}[h!]
\centering
\caption{\textbf{Fixed Partners in the Matched Sample}}
\footnotesize
\setlength{\tabcolsep}{8pt}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{l c c}
