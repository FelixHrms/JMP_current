...
The yield reaction to the position adjustment depends on the market's ability to absorb it. The trading pressure equals the proportional adjustment multiplied by the share of the bond that the hedge fund sector holds, and its price impact depends on the depth of the market for that bond. Bonds in which hedge funds hold leveraged positions should therefore react more strongly to a shock than otherwise comparable bonds, and the more so the larger the position relative to the amount outstanding.

Under the framework, the additional yield move reflects trading pressure rather than a change in fundamentals. It should therefore not persist, and it should revert as the adjustment is completed and other capital absorbs the imbalance.

The two sides of the position adjustment differ in their nature. The contraction is enforced by the requirement to keep meeting margin, while the expansion is voluntary. The contraction is the deleveraging response studied in the literature on funding constraints, in which losses erode capital and force position reductions \citep{brunnermeier2009, geanakoplos2010}. The expansion is its counterpart under the balance sheet management of \citet{adrian2014}, in which freed capacity is redeployed into the existing exposure. Such redeployment is also the optimal response of a constrained arbitrageur whose desired position exceeds its funding capacity, because a favorable surprise enlarges the capacity while the remaining expected return still justifies a larger position than the enlarged balance sheet permits \citep{gromb2002}, and hedge funds are indeed documented to raise leverage when asset values rise and funding costs fall \citep{ang2011}. Appendix \ref{subsec:calibration} works through a numerical example of both sides. Under either reading, favorable and unfavorable shocks should produce amplification of comparable size, with a position contraction in the one case and an expansion in the other.

The reasoning applies to books with net exposure to the shock. A duration-hedged book gains or loses little capital from a level move because its legs offset, so the mechanism assigns it neither a forced contraction nor a voluntary expansion, and the additional reaction should be absent for such books. Section \ref{sec:directionality} exploits this distinction and discusses how hedged books appear in the repo data.

In sum, the framework implies that bonds in which hedge funds hold leveraged positions react more strongly to a shock than comparable bonds, the more so the larger the position relative to the amount outstanding, that the additional reaction is temporary, that it arises for shocks in both directions with position adjustments of opposite sign, and that it is absent for hedged books. Section \ref{subsec:amplification} examines the first two implications, Section \ref{subsec:asymmetries} the third, and Section \ref{sec:directionality} the fourth. The results are read as consistent with the framework rather than as establishing it, and Section \ref{subsec:asymmetries} returns to the question of alternative explanations.
