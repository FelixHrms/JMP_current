...
In sum, the framework implies that bonds in which hedge funds hold leveraged positions react more strongly to a shock than comparable bonds, the more so the larger the position relative to the amount outstanding, that the additional reaction is temporary and reverts as other capital absorbs the imbalance, and that the amplification is symmetric, with a position contraction when the shock moves prices against the position and an expansion when it moves in its favor. Section \ref{subsec:amplification} examines the first two implications and Section \ref{subsec:asymmetries} the third. The results are read as consistent with the framework rather than as establishing it, and Section \ref{subsec:asymmetries} returns to the question of alternative explanations.
...
The evidence also speaks against several alternative explanations of this pattern. A higher sensitivity of hedge fund bonds to the shock for reasons unrelated to leveraged trading, such as the selection of more sensitive bonds, would produce symmetric coefficients as well. It would not produce a differential that reverts within days (Figure \ref{fig:baseline_persistence}), that varies with the size of the position within the same bond (Section \ref{subsec:amplification}), or that is confined to directional books (Section \ref{sec:directionality}). Faster price discovery by informed investors would imply a lasting move toward fundamentals and a stronger effect for information surprises, whereas the differential overshoots and reverts and is concentrated in pure monetary surprises (Table \ref{tab:jk}). A dealer-driven explanation is inconsistent with the results under dealer-by-date fixed effects and with the absence of any scaling with dealer stress (Table \ref{tab:dealer_robustness}). What the evidence does not rule out is trend-following on the relaxing side, since a fund that adds to a winning position for reasons unrelated to its capital trades in the same direction. The link to position size and the absence of an effect for hedged books favor the capital-based reading, but they do not exclude this alternative.
