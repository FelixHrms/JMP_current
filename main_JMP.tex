...
\indent \textbf{Disclaimer: } I am grateful to Michael Bauer, Lennart Dekker, Daniel Dieckelmann, Maurizio Habib, Vasso Ioannidou, Christoph Kaufmann, Loriana Pelizzon, Maik Schmeling, Fabricius Somogyi, Valeri Sokolovski, Davide Tomio, Charles O'Donnell and participants of the EFA Doctoral Tutorial, ECB Seminar Series, IFABS 2026, and the 2026 Aarhus Finance Forum for helpful comments and suggestions. This paper received the best PhD Paper Award at the IFABS 2026 Conference. This paper should not be reported as representing the views of the European Central Bank (ECB). The views expressed are those of the author and do not necessarily reflect those of the ECB.}
\vspace{3cm}
...
\paragraph{The relaxing side.} The margin call makes the constraining side mechanical. The relaxing side is not. When the same 6 basis point surprise moves in favor of the position, the bond gains $0.42\%$, the fund receives the corresponding variation margin in cash, and its capital buffer rises by 21\%. Nothing compels the fund to redeploy this capacity, so the symmetric expansion cannot be taken for granted.

To see when redeployment is nonetheless the natural response, consider the same fund when its position reflects a view on the path of policy rates. Suppose the fund expects the yield of the seven year bond to fall 25 basis points below what is currently priced and holds the bond at the maximum size its margin permits. At the effective margin of 2\%, the expected move corresponds to a return on capital of approximately $25 \times 7 \times 0.5 \approx 88\%$, which is why the position sits at the funding constraint rather than at an interior optimum: the fund would hold more if its capacity allowed it. A favorable 6 basis point surprise then does two things at once. It consumes part of the perceived gap, leaving 19 basis points, and it raises the fund's margin capacity by 21\%. The remaining gap still corresponds to an expected return on capital of roughly $19 \times 7 \times 0.5 \approx 66\%$, so the position justified by the view continues to exceed what the balance sheet permits, and a fund in this situation expands to its new capacity, buying an additional 21\% of its position. This is the same magnitude that the margin call forces on the constraining side, with the opposite trigger. Both regimes are then governed by a single rule: the position equals the smaller of the funding capacity and the position justified by the remaining view. As long as the view stays ahead of the market, favorable and unfavorable shocks alike move the fund along the capacity branch, and the position adjustment is symmetric with opposite sign. Once prices approach the view, the rule switches branches: the fund stops adding and eventually unwinds, so view-driven expansion is self-limiting rather than open-ended. This position behavior corresponds to that of margin-constrained arbitrageurs, whose holdings track their capital whenever desired positions exceed funding capacity \citep{gromb2002, brunnermeier2009}, and hedge fund leverage is documented to be actively and procyclically managed \citep{ang2011}.

Two remarks qualify this reasoning. First, the expansion, unlike the contraction, is a choice. The example shows that expanding is consistent with the optimal behavior of a constrained fund whose view the surprise has not exhausted, not that it is compelled, and the empirical analysis accordingly treats symmetric amplification as a prediction to be tested rather than as an implication of margin mechanics alone. Second, the reasoning applies to books with net exposure to the shock. A duration-hedged book, such as a cash-futures basis position, gains little capital from a level move because its legs offset, so the mechanism assigns it neither a forced contraction nor a voluntary expansion. Section \ref{sec:directionality} exploits exactly this distinction, and discusses how hedged books appear in the repo data.
