...
To interpret trading patterns, I first map repo market activity to the underlying directional exposure of hedge funds. Since hedge funds use the repo market primarily to finance leveraged positions or source securities for short selling, the direction of the repo transaction serves as a proxy for their trading position. Specifically, I classify net cash borrowing (repo) as a proxy for leveraged long positions, where the fund posts the bond as collateral to finance its purchase. Conversely, I classify net cash lending (reverse repo) as a proxy for short positions, where the fund borrows the specific bond to sell it in the cash market. This mapping is supported by the composition of the sector. Classifying each of the 152 funds by investment strategy shows that fixed income relative value and global macro funds, strategies for which the repo transaction is an integral part of the sovereign bond trade itself rather than a source of funding for unrelated positions, account for 63\% of funds and 85\% of repo volume (Appendix \ref{subsec:classification} details the classification and shows that the results are unchanged when positioning is measured from these funds only). Other instruments such as futures, total return swaps, or options might also provide leveraged sovereign bond exposure, but are not directly observable in the SFTDS.
...
This section describes the framework that guides the empirical analysis. It traces how a price shock to a leveraged position affects the capital of the fund and, through the resulting position adjustment, the yield of the bond, and it derives the predictions that the following sections test. The framework is intended to motivate the empirical design rather than to serve as a formal model, and the contribution of the paper is empirical. Two features of the framework should be noted at the outset. First, the empirical tests are the same for shocks that move prices against a position and for shocks that move prices in its favor. In both cases, the data reveal whether positions adjust in the predicted direction and whether yields react more strongly, but not the motive behind the adjustment. Second, the two cases differ in the assumptions required to predict the adjustment. When a shock moves prices against the position, the margin requirement implies the contraction without any assumption about the objective of the fund. When a shock moves prices in its favor, the expansion requires the assumption that the desired position of the fund exceeds what its balance sheet permits. Appendix \ref{subsec:calibration} presents a calibrated version of the framework based on sample moments.
...
Policy surprises of the size observed in the sample have a sizable effect on the capital buffer. The average tightening surprise in the sample is approximately 6 basis points, which moves the price of a representative bond with a duration of about seven years by roughly 0.42\%. Under an effective margin of 2\%, which is conservative relative to the haircuts reported in Table \ref{tab:summary_stats}, Equation (\ref{eq:capital}) implies that a single surprise moves the capital buffer by about a fifth.

The fund keeps the position aligned with the capital backing it. A shock that moves prices against the position erodes the buffer and tightens the balance sheet constraint, forcing position reduction. A shock that moves prices in favor of the position relaxes the constraint and frees capacity for additional risk-taking. Both responses are thus governed by the same rule, with the sign of the shock in combination with the position of the fund determining whether positions contract or expand. In both cases, the resulting trading pushes yields further in the direction of the initial shock. The two sides differ in their nature, however. The contraction is enforced by the requirement to keep meeting margin, while the expansion is voluntary. When a surprise moves in favor of the position, the fund receives the corresponding variation margin in cash, and nothing compels it to redeploy this capacity.

Redeployment is nonetheless the optimal response for a fund whose desired position exceeds its funding capacity. Consider a fund that expects yields to fall below what is currently priced and holds the bond at the maximum size its margin permits. The expected return on the capital backing the position is then large, and the position is at the funding constraint rather than at an interior optimum. The fund would hold more if its capacity allowed it. A favorable surprise has two effects. It consumes part of the expected return and it raises the capacity of the fund. As long as the remaining expected return justifies a larger position than the balance sheet permits, capacity remains the binding constraint, and the fund expands its position by the same proportion by which the capital erosion reduces it on the constraining side.\footnote{In the calibrated example of Appendix \ref{subsec:calibration}, a favorable 6 basis point surprise lowers the expected return on capital from about 88\% to about 66\%, so the position justified by the view continues to exceed what the balance sheet permits.} Once yields reach the view, the expected return is exhausted. The fund stops adding and eventually unwinds, so the expansion ends on its own. This behavior corresponds to that of arbitrageurs facing margin constraints, whose holdings track their capital whenever desired positions exceed funding capacity \citep{gromb2002, brunnermeier2009}, and hedge funds are documented to raise leverage when asset values rise and funding costs fall \citep{ang2011}. Leverage management toward a target ratio, as \citet{adrian2014} document for US broker-dealers, produces the same two-sided response, and the analysis below does not distinguish between these motives.

The yield reaction to the position adjustment depends on the market's ability to absorb it. The trading pressure equals the proportional adjustment multiplied by the share of the bond that the hedge fund sector holds, and its price impact depends on the depth of the market for that bond. Amplification should therefore increase with the size of the position relative to the amount outstanding. It should also be temporary, because the additional yield move reflects trading pressure rather than a change in fundamentals, and it should revert as the adjustment is completed and other capital absorbs the imbalance.

The reasoning applies to books with net exposure to the shock. A duration-hedged book gains or loses little capital from a level move because its legs offset, so the mechanism assigns it neither a forced contraction nor a voluntary expansion. Section \ref{sec:directionality} exploits exactly this distinction, and discusses how hedged books appear in the repo data.

The framework yields four predictions that structure the remainder of the paper.

\begin{enumerate}[label=(P\arabic*), ref=P\arabic*, leftmargin=*]
\item \label{pred:amplification} Bonds in which hedge funds hold leveraged positions react more strongly to a monetary policy surprise than otherwise comparable bonds, and the differential increases with the size of the position relative to the amount outstanding.
\item \label{pred:symmetry} The differential is present both when the surprise moves prices against the position and when it moves in its favor. The underlying position adjustments have opposite signs, a contraction in the first case and an expansion in the second.
\item \label{pred:reversion} The differential is temporary. It appears on the announcement day and reverts within days, because it reflects trading pressure rather than a difference in fundamentals.
\item \label{pred:hedged} The differential is confined to books with net exposure to the shock and is absent for duration-hedged books.
\end{enumerate}

Section \ref{subsec:amplification} tests Predictions \ref{pred:amplification} and \ref{pred:reversion}, Section \ref{subsec:asymmetries} tests Prediction \ref{pred:symmetry}, and Section \ref{sec:directionality} tests Prediction \ref{pred:hedged}. The predictions concern the sign, symmetry, timing, and incidence of the yield response and the direction of the position response. They do not concern the motive behind the position adjustment, which Section \ref{subsec:asymmetries} discusses in light of the evidence.
...
Appendix \ref{subsec:calibration} compares this magnitude with the calibrated version of the framework in Section \ref{sec:mechanism}. Based on the haircuts and holdings observed in the sample and on market depth estimates from the literature, the position adjustment implied by a six basis point surprise generates an additional yield change of approximately 3 basis points, which is of the same order of magnitude as the estimate of 1.73 basis points.
