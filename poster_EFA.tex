...
\begin{minipage}[c]{230mm}
  \includegraphics[width=230mm]{Figures/tradeweb.png}
\end{minipage}\hfill
\begin{minipage}[c]{355mm}\fBody
\begin{itemize}
  \item Since 2020 the share of hedge fund trading volume in Europe rose from a third to \textbf{more than half}.
  \item The rise is broad and also evident in the US and the UK.
...
\Message{Research question: Does the leveraged positioning of hedge funds amplify sovereign bond yield sensitivity to shocks? Two competing priors:}
...
\vspace{5mm}
\begin{tcolorbox}[colback=pale,colframe=pale,arc=3mm,left=9mm,right=9mm,top=5.5mm,bottom=5.5mm]
  \centering{\fHead\bfseries\color{navy}Finding: The leveraged positioning of hedge funds amplifies the response of sovereign bond yields to shocks, leading to temporary overshooting.\par}
\end{tcolorbox}
\end{Panel}
...
\begin{minipage}[t]{200mm}
  ...
  {\fBody Transaction-level data on euro area repo activity in German and Italian sovereigns from Jan 2021 to Nov 2025. Repo proxies long positions and reverse repo proxies short positions.\par}
\end{minipage}\hfill
\begin{minipage}[t]{195mm}
  ...
  {\fBody Daily yields, duration, bid-ask spread, amount issued, and cheapest-to-deliver (CTD) status per bond.\par}
\end{minipage}\hfill
\begin{minipage}[t]{200mm}
  ...
  {\fBody Intraday 2-year OIS changes in the ECB monetary event window. The same shock hits every bond.\par}
\end{minipage}

\vspace{9mm}
\begin{tcolorbox}[colback=pale,colframe=pale,arc=3mm,left=9mm,right=9mm,top=6.5mm,bottom=6.5mm]
  \centering\fBody
  $\Delta \mathrm{yield}_{i,t} \;=\; {\color{rust}\beta_1\,(\mathrm{HF}_{i,t-1} \times \mathrm{Shock}_{t})} \;+\; \beta_2\, \mathrm{HF}_{i,t-1} \;+\; \beta_3\, \mathrm{Shock}_{t} \;+\; \gamma\, \mathbf{X}_{i,t-1} \;+\; \theta_{k} \;+\; \varepsilon_{i,t}$
  \par\vspace{3.5mm}
  {\fSmall ${\color{rust}\beta_1}$ measures the additional yield sensitivity to shocks when hedge funds are active in a given bond.\par}
\end{tcolorbox}
...
\begin{itemize}
\item Hedge fund positions measured either as a binary activity indicator or as a continuous intensity measure (five-day net position scaled by the amount outstanding).
  \item Selection problem: HFs select liquid, large, long-duration, CTD bonds.
...
\begin{tabular}{c @{\hspace{24mm}} c}
  ...
  {\fSmall ECB announcements} & {\fSmall of an issue held by HFs when active}\\
\end{tabular}
\end{minipage}
\end{Panel}
...
\begin{minipage}[c]{226mm}
  \includegraphics[width=226mm]{Figures/intensity_volatility_bars.png}
  \FigNote{Average $|\Delta$yield$|$ (bp) on non-shock days by quartile of hedge fund intensity.}
\end{minipage}\hfill
\vspace{5mm}
\begin{minipage}[c]{310mm}
  ...
  \begin{tabular}{l c c}
    ...
    \bottomrule
  \end{tabular}
  \FigNote{\centering Standard errors clustered by date and bond. $^{***}$ $p<0.01$.\par}
\end{minipage}

\Message{Volatility rises with hedge fund involvement unconditionally, but among comparable bonds the excess volatility is confined to shock days.}
\end{Panel}
...
\begin{minipage}[c]{300mm}
  ...
  \begin{tabular}{l c c c}
    ...
    \bottomrule
  \end{tabular}
  \FigNote{\centering HF intensity is the log of one plus the five-day net position in percent of the amount outstanding. Standard errors clustered by date and bond, in column (2) by cell and twin. $^{***}$ $p<0.01$.\par}
\end{minipage}\hfill
\begin{minipage}[c]{270mm}
  \centering
  \includegraphics[width=262mm]{Figures/IR_baseline_decomposition_overshoot.png}
  \FigNote{Cumulative yield response per bp of shock, relative to the common level where both groups settle (local projections, matched sample, Jord\`a 2005). Shaded areas are 90\% confidence intervals.}
\end{minipage}
\vspace{5mm}

\Message{More than a quarter of the shock-day yield move reflects positioning, and the temporary overshooting points to amplification rather than price discovery.}
\end{Panel}
...
\begin{minipage}[c]{280mm}
  ...
  \begin{tabular}{l c c}
    ...
    \bottomrule
  \end{tabular}
  \FigNote{\centering In the constraining regime the shock moves against the position and in the relaxing regime in its favor. A Wald test cannot distinguish the two ($p=0.16$). Standard errors clustered by date and bond. $^{***}$ $p<0.01$.\par}
\end{minipage}\hfill
\begin{minipage}[c]{270mm}
  \centering
  \includegraphics[width=262mm]{Figures/IR_on_momentum.png}
  \FigNote{Cumulative change in hedge fund intensity after the shock for overnight repo positions (local projections, Jord\`a 2005). Shaded areas are 90\% confidence intervals.}
\end{minipage}
\vspace{5mm}
...
\begin{minipage}[c]{272mm}
  \centering
  \includegraphics[width=258mm]{Figures/motivation_chart.png}
  \FigNote{Aggregate net repo positions of hedge funds in German (DE) and Italian (IT) sovereigns. Negative values indicate short positions and positive values long positions.}
\end{minipage}\hfill
\begin{minipage}[c]{300mm}
  \centering
  {\fTab $\mathrm{Dir}_{f} \;=\; \left|\textstyle\sum_{i} D_{i}\, q_{f,i}\right| \Big/ \textstyle\sum_{i} D_{i}\, \left|q_{f,i}\right|$
   \qquad
   $\mathrm{HolderDir}_{i} \;=\; \textstyle\sum_{f} \left|q_{f,i}\right| \mathrm{Dir}_{f} \Big/ \textstyle\sum_{f} \left|q_{f,i}\right|$\par}
  \vspace{2mm}
  {\fTiny\color{midgrey}Fund directionality $\mathrm{Dir}_{f}$ is zero when long and short legs offset in duration terms and one when the book is one-sided, with $D_{i}$ the duration and $q_{f,i}$ the net position in bond $i$. The analysis uses bond-level holder directionality $\mathrm{HolderDir}_{i}$, the position-weighted average across the funds holding the bond.\par}
  \vspace{4.5mm}
  \fTab
  \begin{tabular}{l c}
    \toprule
    ...
    \bottomrule
  \end{tabular}
  \FigNote{\centering Hedged and directional books split at the median of holder directionality, quartiles Q1 to Q4 sort by the same measure. Standard errors clustered by date and bond. $^{***}$ $p<0.01$.\par}
\end{minipage}
...
\begin{tcolorbox}[colback=navy,colframe=navy,sharp corners,boxrule=0pt,
    width=716mm,left=53mm,right=53mm,top=8mm,bottom=24mm]
