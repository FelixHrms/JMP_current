...
\begin{minipage}[c]{300mm}
  ...
  \vspace{4.5mm}
  \centering
  {\fTab $\mathrm{Dir}_{f} \;=\; \left|\textstyle\sum_{i} D_{i}\, q_{f,i}\right| \Big/ \textstyle\sum_{i} D_{i}\, \left|q_{f,i}\right|$\par}
  \vspace{2mm}
  {\fTiny\color{midgrey}Zero when long and short legs offset in duration terms and one when the book is one-sided, with $D_{i}$ the duration and $q_{f,i}$ the net position in bond $i$. Each bond inherits the position-weighted average directionality of its holders.\par}
  \vspace{4.5mm}
  \fTab
  \begin{tabular}{l c}
    \toprule
