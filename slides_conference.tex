...
%
% Cut down from the 60 minute seminar deck. 14 numbered content frames in the
% main part, everything else lives in the annex and is reachable through the
% navigation buttons for Q&A.
%
% Suggested timing (about 20 minutes)
%   1  Footprint ........................... 1.0
%   2  Two views ........................... 1.5
%   3  This paper (question + findings) .... 1.5
%   4  Data ................................ 1.5
%   5  Mechanism ........................... 1.5
%   6  Specification and identification .... 1.5
%   7  Finding 1, weak vs strong design .... 1.5
%   8  Finding 2, tightest designs ......... 2.0
%   9  Magnitude ........................... 1.0
%  10  Overshooting (two step build) ....... 2.0
%  11  Finding 3, symmetry ................. 1.5
%  12  Finding 4, net positions ............ 1.0
...
% --- Scene setter: the growing hedge fund footprint (as in the seminar deck) ---
...
\end{columns}
\vfill
\takeaway{Do hedge funds change how prices incorporate new information?}
\end{frame}


% --- Two opposing views (as in the seminar deck) ---
\begin{frame}{Two views on the impact of leveraged investors on price}
\lvs
{\itshape ``On the one hand, hedge funds are frequently credited with \higr{enhancing market efficiency} and liquidity. On the other hand, they could also act as potential \hird{amplifiers of volatility and market shocks}.''}\\[3pt]
{\scriptsize\color{softgrey} The ECB Blog, \textit{Hedge funds: good or bad for market functioning?}, September 2024}
\svs
\begin{columns}[t]
...
\lead{Approach}
\bit\itemsep2pt
  \item Interact \hibl{predetermined} hedge fund positions in individual sovereign bonds with high frequency \hibl{monetary policy surprises}
  \item The interaction recovers the additional yield sensitivity that arises when hedge funds trade a given bond
\eit
\svs
\lead{Four findings}
\ben\itemsep2pt
...
%========================================================================================
%	FINDING 1 (1 slide, weak vs strong design; full table and raw pattern in the annex)
%========================================================================================
% Verbal cues: col 1 raw -> HF bonds more volatile everywhere (level 0.459) and more
% responsive to shocks; col 2 within dur x country x date cells -> level collapses to
% insignificant, interaction unchanged. The differential is shock conditional.
...
\toprule
\textbf{$|\Delta$ yield$|$ (bps)} & (1) Raw comparison & (2) Comparable bonds \\
\midrule
\rowcolor{goetheblue!8}
HF involved $\times$ $|$Shock$|$ & 0.240$^{***}$ & 0.246$^{***}$ \\
...
\end{tabular}}
\end{center}
\fignote{Dependent variable is the absolute daily yield change. Column 2 identifies the effect within cells of bonds with similar duration from the same issuer on the same day. Intermediate specifications in the annex. Standard errors clustered by date and bond. $^{***}\,p<0.01$.}
\begin{columns}[t]
\begin{column}{0.5\textwidth}
{\footnotesize In the raw comparison hedge fund bonds are more volatile on \hird{every} day, hedge funds select volatile bonds.}
\end{column}
\begin{column}{0.5\textwidth}
{\footnotesize Among comparable bonds the \hird{level} differential disappears while the \hibl{interaction} with the shock survives.}
\end{column}
\end{columns}
\svs
\takeaway{The excess volatility is shock conditional, so what changes is how yields react to shocks}
\navbtnTwo{voltable}{Full table}{volbars}{Raw pattern}
\end{frame}


%========================================================================================
%	FINDING 2 (tightest designs, magnitude, overshooting; binned scatter in the annex)
%========================================================================================
% Verbal cues: presence stable 0.291/0.304 in the two tightest designs (dur x ctry x
% date FE, matched twins); intensity 0.164 within the same bond -> dose response.
% Intensity is the measure Finding 3 continues with.
...
\textbf{$\Delta$ yield (bps)} & (1) & (2) Matched & (3) \\
\midrule
\rowcolor{goetheblue!8}
HF involved $\times$ Shock  & 0.291$^{***}$ & 0.304$^{***}$ &  \\
\rowcolor{goetheblue!8}
...
Bond FE                     & Yes & No & Yes \\
Dur. $\times$ Date $\times$ Country FE & Yes & No & Yes \\
Matched pair FE             & No & Yes & No \\
\bottomrule
\end{tabular}}
\end{center}
\fignote{Only the two tightest designs and the continuous dose response, all weaker specifications in the annex. HF intensity is the log of one plus the five day net position scaled by amount outstanding. Column 2 pairs each hedge fund bond with the twin closest in liquidity within its cell. Standard errors clustered by date and bond, and by cell and twin in column 2. $^{***}\,p<0.01$.}
Presence and intensity point the same way. Within the \hibl{same bond} over time, a larger position produces a larger reaction, which pure selection cannot explain.
\navbtnTwo{basefull}{All specifications}{dealer}{Not dealers}
\end{frame}

\begin{frame}{The amplification is economically large}
\hypertarget{magnitude}{}
\svs
Take the interaction estimate from the tightest design to the average tightening surprise.
\lvs
\begin{center}
\begin{tikzpicture}[font=\small]
  \node[greybox, text width=3.4cm] (surp) {Average tightening surprise\\[3pt]{\large\textbf{$\approx 6$ bps}}};
  \node[right=4mm of surp] (times) {\large $\times$};
  \node[keybox, text width=3.4cm, right=4mm of times] (coef) {Interaction estimate\\[3pt]{\large\textbf{0.29 bps per bp}}};
  \node[right=4mm of coef] (eq) {\large $=$};
  \node[redbox, text width=3.4cm, right=4mm of eq] (extra) {Additional move on hedge fund bonds\\[3pt]{\large\hird{\textbf{$\approx 1.7$ bps}}}};
\end{tikzpicture}
\end{center}
\svs
{\centering\scriptsize\color{softgrey} Relative to a total shock day move of about 6.3 bps on these bonds, from the unsaturated specification in the annex.\par}
\lvs
...
\navbtnTwo{scatterbin}{Binned scatter}{extval}{External validity}
\end{frame}
...
% Two-step build as in the seminar: panel (a) alone first; a click adds panel (b),
% the title suffix, the takeaway, and the annex button.
\begin{frame}{Yields diverge after a shock\alt<2->{, reflecting temporary overshooting}{\hphantom{, reflecting temporary overshooting}}}
\hypertarget<2>{overshoot}{}
\begin{columns}[c]
\begin{column}{0.5\textwidth}
\centering
...
\begin{column}{0.5\textwidth}\onslide<2->{{\footnotesize Both legs converge to a common level. Hedge fund bonds \hird{overshoot} first.}}\end{column}
\end{columns}
\onslide<2->{\takeaway{A temporary dislocation that other capital gradually absorbs, consistent with slow moving capital}}
...
\svs
The same \hibl{intensity} specification as before, now estimated separately by regime.
\svs
\begin{columns}[c]
\begin{column}{0.46\textwidth}
\centering
...
%========================================================================================
%	FINDING 4 (2 slides, measure and trends in the annex)
%========================================================================================
% --- The aggregate net positions graph opens the finding (memorable scene setter) ---
\begin{frame}{Finding 4: The directionality of the book governs the strength}
\svs
\begin{center}
\measureplot{0.062}{0.991}{\includegraphics[width=0.72\textwidth,height=0.56\textheight,keepaspectratio]{Figures/motivation_chart.png}}
\end{center}
...
\begin{columns}[c]
\begin{column}{0.5\textwidth}
\lead{Bond level holder directionality}
\beq
...
\end{columns}
\lvs
\takeaway{The amplification rises monotonically with directionality and is not distinguishable from zero in the most hedged quartile}
\navbtnTwo{dirmeasure}{Whole book intuition}{dirtrend}{Time series}
\end{frame}
...
% --- Raw volatility pattern (linked from the Finding 1 slide) ---
\begin{frame}{Raw volatility by hedge fund intensity}
\hypertarget{volbars}{}
\svs
\begin{center}
...
\takeaway{Unconditionally, hedge funds sit in more volatile bonds}
\navbtn{f1}{Back}
\end{frame}

% --- Full presence table (linked from the Finding 2 slide) ---
...
\begin{frame}{Hedge fund presence amplifies the yield reaction, all specifications}
\hypertarget{basefull}{}
\vspace{0.2cm}
\begin{center}
\scriptsize
...
\navbtnTwo{intensity}{Dose response}{baseline}{Back}
\end{frame}

% --- Binned scatter (linked from the magnitude slide) ---
\begin{frame}{The reaction to the same surprise, with and without hedge funds}
\hypertarget{scatterbin}{}
\svs
\begin{center}
